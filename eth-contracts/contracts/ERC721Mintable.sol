...
contract Ownable {
    ...
    modifier onlyOwner() {        
        require(_owner == msg.sender , " it's owner") ;
        _;
    }
...
//  2) create a public setter using the inherited onlyOwner modifier 
    function isPaused(bool p)public onlyOwner{
        if (p == true){
            emit Paused();
        }else{
            emit Unpaused();
    ...
    function approve(address to, uint256 tokenId) public {
        ...
        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
            if(isApprovedForAll() != true){
                require(msg.sender == ownerOf(tokenId));// not sure 
            }
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function setTokenURI(uint32 tokenId) internal{
...
contract FahadERC721Token is ERC721Metadata ("FahadERC721Token","FERCT","https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/") {
...
//      -calls the superclass mint and setTokenURI functions
    function mint(address to , uint32 tokenId , string memory tokenURI) public onlyOwner returns (bool){ 
        _isCompleted = true ; 
        _mint(to , tokenId );
