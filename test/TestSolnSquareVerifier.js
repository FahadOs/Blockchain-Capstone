...
contract('SolnSquareVerifier', accounts => {
    ...
    describe('SolnSquareVerifier', function () {
        ...
        it('Test if an ERC721 token can be minted for contract a new solution can be added for contract - SolnSquareVerifier', async function () {
            let canBeMinted = await this.contract.mintNewNFT(account2,
                                                            2,
                                                            proof["proof"]["A"],
                                                            proof["proof"]["A_p"],
                                                            proof["proof"]["B"],
        ...
        it('Test that a new solution cannot be added if the proof was used previously', async function () {
            let isAlreadyAdded = false;
            try{
                await this.contract.addSolution(account2,
                    2,
                    proof["proof"]["A"],
                    proof["proof"]["A_p"],
                    proof["proof"]["B"],
            ...
            assert.equal(isAlreadyAdded, true, "The proof was used previously");
        });
